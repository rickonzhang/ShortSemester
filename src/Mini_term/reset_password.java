...
public class reset_password implements ActionListener {
	JFrame frame;
	...
	public reset_password(JFrame aFrame) {
		this.superFrame = aFrame;
		frame = new JFrame("用户密码重置");
		frame.add(pic_panel);
		pic_panel.setLayout(new GridLayout(3,1));
		
		label = new JLabel("请输入需要重置密码的用户编号",JLabel.CENTER);
