...
public class user_inforModify implements ActionListener {
	JFrame admin_Frame;
	...
	JButton button;
	JCheckBox name_CheckBox,family_CheckBox,gender_CheckBox,phone_CheckBox,email_CheckBox;
	...
	public user_inforModify(JFrame aFrame) {
		this.superFrame = aFrame;
		admin_Frame = new JFrame("用户信息更改");
		...
		/*name_CheckBox = new JCheckBox();
		name_Panel.add(name_CheckBox);
		name_CheckBox.addChangeListener(this);*/
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source.equals(button)) {
			...
			infor[2] = (String) family_ComboBox.getSelectedItem();
			infor[3] = (String) gender_ComboBox.getSelectedItem();
			infor[4] = phone_TextField.getText();
			infor[5] = email_TextField.getText();
		}
		for(int i = 0; i < 6 ; i++){
			if(infor[i].equals("")){
				guide = 1;
		...
		if( guide == 1 ) {
		    ...
		    new_User.setuName(infor[1]);
		    new_User.setFamily_fID(Family.nametoid(infor[2]));
		    new_User.setGender(infor[3]);
		    new_User.setPhone(infor[4]);
		    new_User.setEmail(infor[5]);
		    Usersmanage.modify(new_User);
		    admin_Frame.dispose();
		    superFrame.dispose();
			new Admin();
		}
